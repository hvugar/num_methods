...
void IProblem2HForward2D::calculateMVD(DoubleMatrix &u, DoubleMatrix &ut, std::vector<SpacePointInfo> &info, bool use) const
{
...
void IProblem2HForward2D::calculateMVD_D(DoubleMatrix &u, DoubleMatrix &ut, std::vector<SpacePointInfo> &info, bool use) const
{
    ...
    for (unsigned int j=0; j<No; j++) IProblem2H2D::distributeDeltaG(mOptParameter.xi[j], j, obsPointNodes, dimX, dimY, 4);
    for (unsigned int i=0; i<Nc; i++) IProblem2H2D::distributeDeltaG(mOptParameter.eta[i], i, cntDeltaNodes, dimX, dimY, 4);
    for (unsigned int s=0; s<Ns; s++) IProblem2H2D::distributeDeltaG(mEquParameter.theta[s], s, qPointNodes, dimX, dimY, 4);
...
//                    }
        ...
        if (use == true) add2Info(u, info, obsPointNodes, l, hx, hy, _INFO_ROWS_, _INFO_COLS_);
...
//void IProblem2HForward2D::calculateMVD_D_(DoubleMatrix &u, DoubleMatrix &ut, std::vector<SpacePointInfo> &info, bool use) const
//{
//    Dimension dimX = spaceDimension(Dimension::DimensionX);
//    Dimension dimY = spaceDimension(Dimension::DimensionY);
//    Dimension time = timeDimension();
...
//    //----------------------------------------------------------------------------------------------//
//    std::vector<SpacePointInfo> obsPointNodes, cntDeltaNodes, qPointNodes;
////    for (unsigned int j=0; j<No; j++) IProblem2H2D::distributeDeltaG(mOptParameter.xi[j], j, obsPointNodes, dimX, dimY, 4);
////    for (unsigned int i=0; i<Nc; i++) IProblem2H2D::distributeDeltaG(mOptParameter.eta[i], i, cntDeltaNodes, dimX, dimY, 4);
////    for (unsigned int s=0; s<Ns; s++) IProblem2H2D::distributeDeltaG(mEquParameter.theta[s], s, qPointNodes, dimX, dimY, 4);
...
//    //        }
//    //    }
//    //    if (use == true) add2Info(u00, info, 0, hx, hy, _INFO_ROWS_, _INFO_COLS_);
//    //    layerInfo(u00, 0);
//    //    layerInfo(u00, u00, 0);
//    //    //------------------------------------- initial conditions -------------------------------------//
//    //    double hh = 0.5*ht;
...
//    //    if (use == true) add2Info(u10, info, 1, hx, hy, _INFO_ROWS_, _INFO_COLS_);
//    //    layerInfo(u10, u10, 1);
//    //    layerInfo(u10, 1);
//    initialLayers(u00, u05, u10, info, use, obsPointNodes, cntDeltaNodes, qPointNodes, N, M, hx, hy, ht, aa__hxhx, aa__hyhy, lambda);
//    //------------------------------------- initial conditions -------------------------------------//
...
//    double *a1X, *b1X, *c1X, *d1X, *x1X;
//    double *a1Y, *b1Y, *c1Y, *d1Y, *x1Y;
//    //mallocVectors(a1X, b1X, c1X, d1X, x1X, N-1, a1Y, b1Y, c1Y, d1Y, x1Y, M-1);

//    mallocVectorsX(a1X, b1X, c1X, d1X, x1X, N-1, 0.0, 0.0);
//    mallocVectorsX(a1Y, b1Y, c1Y, d1Y, x1Y, M-1, 0.0, 0.0);
...
//        }
//    }
//    if (use == true) add2Info(u00, info, obsPointNodes, 0, hx, hy, _INFO_ROWS_, _INFO_COLS_);
//    layerInfo(u00, 0);
//    layerInfo(u00, u00, 0);
//    //------------------------------------- initial conditions -------------------------------------//
...
//    if (use == true) add2Info(u10, info, obsPointNodes, 1, hx, hy, _INFO_ROWS_, _INFO_COLS_);
//    layerInfo(u10, u10, 1);
//    layerInfo(u10, 1);
//    //------------------------------------- initial conditions -------------------------------------//
                                   ...
                                   unsigned int ln, double hx, double hy, unsigned int rows, unsigned int cols) const
{
    ...
    {
        for (unsigned int j=0; j<info.size(); j++) info[j].u[ln] = info[j].ux[ln] = info[j].uy[ln] = 0.0;

        for (unsigned int opj=0; opj<obsPointNodes.size(); opj++)
        {
            const ExtendedSpacePointNode &opn = obsPointNodes[opj];
            const SpacePointInfo &ui = info[opn.id];
...
void IProblem2HForward2D::mallocVectorsX(double *&ax, double *&bx, double *&cx, double *&dx, double *&rx, unsigned int N, double k0, double k1) const
{
    ...
    for (unsigned int n=0; n<N; n++) ax[n] = k0;
    for (unsigned int n=0; n<N; n++) bx[n] = k1;
    for (unsigned int n=0; n<N; n++) cx[n] = k0;

    ax[0] = cx[N-1] = 0.0;
}

void IProblem2HForward2D::mallocVectorsY(double *&ay, double *&by, double *&cy, double *&dy, double *&ry, unsigned int M, double k0, double k1) const
{
    ay = (double *) malloc(sizeof(double)*M);
    by = (double *) malloc(sizeof(double)*M);
    cy = (double *) malloc(sizeof(double)*M);
    dy = (double *) malloc(sizeof(double)*M);
    ry = (double *) malloc(sizeof(double)*M);

    for (unsigned int m=0; m<=M-1; m++) ay[m-1] = k0;
    for (unsigned int m=0; m<=M-1; m++) by[m-1] = k1;
    for (unsigned int m=0; m<=M-1; m++) cy[m-1] = k0;
...
void IProblem2HForward2D::prepareInfo(unsigned int No, const std::vector<SpacePoint> &points, std::vector<SpacePointInfo> &info,
                                      unsigned int L, const Dimension &dimX, const Dimension &dimY) const
{
    info.resize(No);
    for (unsigned int j=0; j<No; j++)
    {
        SpacePointInfo &inf = info[j];
