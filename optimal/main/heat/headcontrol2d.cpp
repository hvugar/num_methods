...
void HeadControl2D::calculateP(const DoubleVector& E, DoubleVector& g)
{
    ...
    x1 = (int)(round(E[0]/h1));
    x2 = (int)(round(E[1]/h2));
    g[0] = 0.5*g1(M*h)*h * ((psi0[x2][x1+1]-psi0[x2][x1-1])/(2.0*h1));
    g[1] = 0.5*g1(M*h)*h * ((psi0[x2+1][x1]-psi0[x2-1][x1])/(2.0*h2));

    x1 = (int)(round(E[2]/h1));
    x2 = (int)(round(E[3]/h2));
    g[2] = 0.5*g2(M*h)*h * ((psi0[x2][x1+1]-psi0[x2][x1-1])/(2.0*h1));
    g[3] = 0.5*g2(M*h)*h * ((psi0[x2+1][x1]-psi0[x2-1][x1])/(2.0*h2));

    x1 = (int)(round(E[4]/h1));
    x2 = (int)(round(E[5]/h2));
    g[4] = 0.5*g3(M*h)*h * ((psi0[x2][x1+1]-psi0[x2][x1-1])/(2.0*h1));
    g[5] = 0.5*g3(M*h)*h * ((psi0[x2+1][x1]-psi0[x2-1][x1])/(2.0*h2));
    ...
    for (unsigned int p=0; p<M; p++)
    {
        ...
        if (k-1 > 0)
        {
            x1 = (int)(round(E[0]/h1));
            x2 = (int)(round(E[1]/h2));
            g[0] += g1(k*h)*h * ((psi0[x2][x1+1]-psi0[x2][x1-1])/(2.0*h1));
            g[1] += g1(k*h)*h * ((psi0[x2+1][x1]-psi0[x2-1][x1])/(2.0*h2));

            x1 = (int)(round(E[2]/h1));
            x2 = (int)(round(E[3]/h2));
            g[2] += g2(k*h)*h * ((psi0[x2][x1+1]-psi0[x2][x1-1])/(2.0*h1));
            g[3] += g2(k*h)*h * ((psi0[x2+1][x1]-psi0[x2-1][x1])/(2.0*h2));

            x1 = (int)(round(E[4]/h1));
            x2 = (int)(round(E[5]/h2));
            g[4] += g3(k*h)*h * ((psi0[x2][x1+1]-psi0[x2][x1-1])/(2.0*h1));
            g[5] += g3(k*h)*h * ((psi0[x2+1][x1]-psi0[x2-1][x1])/(2.0*h2));
        }
    }

    x1 = (int)(round(E[0]/h1));
    x2 = (int)(round(E[1]/h2));
    g[0] += 0.5*g1(0*h)*h * ((psi0[x2][x1+1]-psi0[x2][x1-1])/(2.0*h1));
    g[1] += 0.5*g1(0*h)*h * ((psi0[x2+1][x1]-psi0[x2-1][x1])/(2.0*h2));

    x1 = (int)(round(E[2]/h1));
    x2 = (int)(round(E[3]/h2));
    g[2] += 0.5*g2(0*h)*h * ((psi0[x2][x1+1]-psi0[x2][x1-1])/(2.0*h1));
    g[3] += 0.5*g2(0*h)*h * ((psi0[x2+1][x1]-psi0[x2-1][x1])/(2.0*h2));

    x1 = (int)(round(E[4]/h1));
    x2 = (int)(round(E[5]/h2));
    g[4] += 0.5*g3(0*h)*h * ((psi0[x2][x1+1]-psi0[x2][x1-1])/(2.0*h1));
    g[5] += 0.5*g3(0*h)*h * ((psi0[x2+1][x1]-psi0[x2-1][x1])/(2.0*h2));
...
double HeadControl2D::m1(double x2, double t) { return u(0.0, x2, t); }
double HeadControl2D::m2(double x2, double t) { return u(1.0, x2, t); }
double HeadControl2D::m3(double x1, double t) { return u(x1, 0.0, t); }
double HeadControl2D::m4(double x1, double t) { return u(x1, 1.0, t); }
...
void HeadControl2D::main()
{
    ...
    for (unsigned int i=0; i<E0.size(); i++) E0[i] = 0.05*(i+1);
...
void HeadControl2DPrinter::print(unsigned int c, const DoubleVector &e, const DoubleVector &s, double alpha, RnFunction *f) const
{
    printf("J: %.16f\n", f->fx(e));
}
