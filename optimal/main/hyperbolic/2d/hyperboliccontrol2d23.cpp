...
double HyperbolicControl2D23::fx(double t)
{
    ...
    v0.clear();
    v0.resize((M+D+1)*L, 0.0);
    printf("t: %f M: %d M+D: %d m %d size: %d\n", t1, M, M+D, m, v0.size());
...
void HyperbolicControl2D23::print(unsigned int i, const DoubleVector &v, const DoubleVector &g, double alpha, RnFunction* fn) const
{
    C_UNUSED(g);
    C_UNUSED(alpha);
