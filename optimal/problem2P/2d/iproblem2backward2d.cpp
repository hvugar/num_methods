...
void IProblem2Backward2D::calculateMVD(DoubleMatrix &p, vector<ExtendedSpaceNode2D> &info, bool use)
{
    ...
    for (unsigned int m=0; m<=M; m++)
    {
        bool found1 = false;
        bool found2 = false;
        for (unsigned int i=0; i<obdeltaNodes.size(); i++)
        {
            const ObservationDeltaNode &odn = obdeltaNodes.at(i);
            if (odn.j == m)
            {
                for (unsigned int j=0; j<controlNodes.size(); j++)
                {
                    const ControlNode &cn = controlNodes.at(j);
                    if (cn.j == m)
                    {
                        found2 = true;
                        break;
                    ...
                    for (unsigned int m=0; m<=M; m++)
                    {
                        ...
                        for (unsigned int onj=0; onj<obdeltaNodes.size(); onj++)
                        {
                            ...
                            if (odn.i == sn.i && odn.j == sn.j)
                            {
                                ...
                                {
                                    const ControlNode &cn = controlNodes[s];
                                    d1Y[m] += ht * mParameter.k[cn.id][odn.id] * ph[cn.j][cn.i] * odn.w * cn.w;
                                }

                                for (unsigned int i=0; i<Lc; i++)
                                {
                                    d1Y[m] -= 2.0 * r * ht *  mParameter.k[i][odn.id] * penalty(i, tn) * odn.w;
                                }
                            }
                        }
                    }
                    ...
                    for (unsigned int n=0; n<=N; n++)
                    {
                        ...
                        for (unsigned int odj=0; odj<obdeltaNodes.size(); odj++)
                        {
                            ...
                            if (odn.i == sn.i && odn.j == sn.j)
                            {
                                ...
                                {
                                    const ControlNode &cn = controlNodes[s];
                                    d1X[n] += ht * mParameter.k[cn.id][odn.id] * p[cn.j][cn.i] * cn.w * odn.w;
                                }

                                for (unsigned int i=0; i<Lc; i++)
                                {
                                    d1X[n] -= 2.0 * r * ht *  mParameter.k[i][odn.id] * penalty(i, tn) * odn.w;
                                }
                            }
                        }
                    }
...
void IProblem2Backward2D::extendControlPoint2(const SpaceNodePDE &eta, std::vector<ControlNode> &cns, unsigned int i) const
{
    ...
    cn.i = rx+2; cn.x = cn.i*hx; cn.j = ry+0; cn.y = cn.j*hy; cn.pt = eta; cn.id = i;
    dx = fabs(cn.x-eta.x); dy = fabs(cn.y-eta.y); cn.w = ((2.0*hx-dx)*(hx-dx)*(3.0*hx-dx)*hx36)*((2.0*hy-dy)*(hy-dy)*(hy+dy)*hy32);
    cns.push_back(cn);
...
void IProblem2Backward2D::extendControlPoint3(const SpaceNodePDE &eta, std::vector<ControlNode> &ons, unsigned int i) const
{
    ...
    distributeDelta(eta, extpoint, i);
...
void IProblem2Backward2D::extendObservationDeltaPoint4(const SpaceNodePDE &xi, std::vector<ObservationDeltaNode> &cps, unsigned int i) const
{
    ...
    for (unsigned int n=x0-3; n<=x0+3; n++)
    {
        ...
        {
            ObservationDeltaNode on;
            on.i = n; on.x = n*hx; on.j = m; on.y = m*hy; on.pt = xi; on.id = i;
