...
void Solver::Main(int argc, char **argv)
{
    ...
    FirstOrderLinearODE fol;
    fol.setDimension(Dimension(0.005, 0, 200));
    std::vector<DoubleVector> rv;
    fol.solveInitialValueProblem(rv, IFirstOrderLinearODE::ODESolverMethod::EULER);

    for (unsigned int i=0; i<=200; i++)
    {
        if (i%20==0) { printf("%10.6f ", rv[i][0]); }
    }
    for (unsigned int i=0; i<=200; i++)
    {
        if (i%20==0) { printf("%10.6f ", rv[i][1]); }
    }
}
...
auto Solver::setDimension(const Dimension &timeDimension, const Dimension &dimensionX, const Dimension &dimensionY) -> void
{
...
auto Solver::fx(double t) const -> double
{
...
auto Solver::integral1(const DoubleMatrix &) const -> double
{
...
auto Solver::integral2(const DoubleMatrix &) const -> double
{
    //const Dimension &time = Problem0HForward::timeDimension();
    const Dimension &dimX = spaceDimensionX();
    const Dimension &dimY = spaceDimensionY();
...
double HeatEquationIBVP::initial(const SpaceNodePDE &sn, InitialCondition cn) const { return solver->frw_initial(sn, cn); }

double HeatEquationIBVP::boundary(const SpaceNodePDE &sn, const TimeNodePDE &tn, BoundaryConditionPDE &cn) const { return solver->frw_boundary(sn, tn, cn); }

double HeatEquationIBVP::f(const SpaceNodePDE &sn, const TimeNodePDE &tn) const { return solver->frw_f(sn, tn); }

void HeatEquationIBVP::layerInfo(const DoubleMatrix &u, const TimeNodePDE &tn) const { return solver->frw_layerInfo(u, tn); }

Dimension HeatEquationIBVP::timeDimension() const
{
...
Dimension HeatEquationIBVP::spaceDimensionY() const { return solver->spaceDimensionY(); }

Dimension HeatEquationIBVP::spaceDimensionZ() const { return solver->spaceDimensionZ(); }

double Solver::frw_initial(const SpaceNodePDE &, InitialCondition) const { return 0.0; }

double Solver::frw_boundary(const SpaceNodePDE &, const TimeNodePDE &, BoundaryConditionPDE & condition) const
{
    condition = BoundaryConditionPDE(BoundaryCondition::Dirichlet, 1.0, 0.0, 0.0);
    return 0.0;
}

double Solver::frw_f(const SpaceNodePDE &sn, const TimeNodePDE &tn) const
{
...
double Solver::p(const SpaceNodePDE &sn, const TimeNodePDE &tn) const
{
...
void Solver::frw_calculateU1U2(const DoubleMatrix &u, const TimeNodePDE &tn) const
{
    const Dimension &time = 0;//forward->timeDimension();
...
double HeatEquationFBVP::final(const SpaceNodePDE &sn, FinalCondition condition) const { return solver->bcw_final(sn, condition); }

double HeatEquationFBVP::boundary(const SpaceNodePDE &sn, const TimeNodePDE &tn, BoundaryConditionPDE &condition) const { return solver->bcw_boundary(sn, tn, condition); }

double HeatEquationFBVP::f(const SpaceNodePDE &sn, const TimeNodePDE &tn) const { return solver->bcw_f(sn, tn); }

void HeatEquationFBVP::layerInfo(const DoubleMatrix &p, const TimeNodePDE &tn) const { return solver->bcw_layerInfo(p, tn); }
...
Dimension HeatEquationFBVP::spaceDimensionY() const { return solver->spaceDimensionY(); }

Dimension HeatEquationFBVP::spaceDimensionZ() const { return solver->spaceDimensionZ(); }

double Solver::bcw_final(const SpaceNodePDE &sn, FinalCondition condition) const
{
...
double Solver::bcw_boundary(const SpaceNodePDE&, const TimeNodePDE&, BoundaryConditionPDE &condition) const
{
    condition = BoundaryConditionPDE(BoundaryCondition::Dirichlet, 1.0, 0.0, 0.0);
    return 0.0;
}
