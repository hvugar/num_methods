...
Problem2HDirichlet2::Problem2HDirichlet2() : Problem2hDirichletBase () {}

Problem2HDirichlet2::~Problem2HDirichlet2()
{}

double Problem2HDirichlet2::integral(const std::vector<DoubleMatrix> &vu) const
{
    ...
    for (unsigned int m=0; m<=M; m++)
    {
        ...
        {
            ut[m][n] = (vu[2][m][n]-vu[0][m][n])/(2.0*ht);
        }
    }
    double sum = integralU(vu[1]) + integralU(ut);

    return sum;
}

auto Problem2HDirichlet2::penalty(const spif_vectorH &info, const OptimizeParameterH &o_prm) const -> double
{
...
auto Problem2HDirichlet2::gradient(const DoubleVector &pv, DoubleVector &g) const -> void
{
    ...
    Problem2HDirichlet2* prob = const_cast<Problem2HDirichlet2*>(this);
...
auto Problem2HDirichlet2::solveForwardIBVP(std::vector<DoubleMatrix> &u, spif_vectorH &u_info, bool use, const DoubleVector &pv, double lambda) const -> void
{
    const Dimension dimX = spaceDimension(Dimension::DimensionX);
    const Dimension dimY = spaceDimension(Dimension::DimensionY);
    const Dimension time = timeDimension();
    ...
    //-------------------------------------------- info --------------------------------------------//
    if (use == true) prepareInfo(No,  mOptParameter.xi, u_info, 2*L+1);
    //----------------------------------------------------------------------------------------------//
    ...
    for (unsigned int ln=2; ln<=(L+1); ln++)
    {
        ...
        if (use == true && ln <= L) add2Info(u15, u_info, 2*ln-1, hx, hy, measuremntGirdList); f_layerInfo(u15, 2*ln-1);
        /**************************************************** x direction apprx ***************************************************/
        /**************************************************** y direction apprx ***************************************************/
        ...
        if (use == true && ln <= L) add2Info(u20, u_info, 2*ln+0, hx, hy, measuremntGirdList); f_layerInfo(u20, 2*ln+0);
        /**************************************************** y direction apprx ***************************************************/
        for (unsigned int m=0; m<=M; m++)
        {
            for (unsigned int n=0; n<=N; n++)
...
auto Problem2HDirichlet2::solveBackwardIBVP(const std::vector<DoubleMatrix> &u, spif_vectorH &p_info, bool use, const spif_vectorH &u_info, const DoubleVector &pv, double lambda) const -> void
{
    const Dimension dimX = spaceDimension(Dimension::DimensionX);
    const Dimension dimY = spaceDimension(Dimension::DimensionY);
    const Dimension time = timeDimension();
    ...
    //-------------------------------------------- info --------------------------------------------//
    if (use == true) prepareInfo(Nc,  mOptParameter.eta, p_info, 2*L+1);
    //-------------------------------------------- info --------------------------------------------//

    //------------------------------------- initial conditions -------------------------------------//
    initConjtWeightMatrix(u);
    /************************************************************************/
    SpaceNodePDE sn00;
    for (unsigned int m=0; m<=M; m++)
    {
    ...
    if (use == true) add2Info(p00, p_info, 2*L, hx, hy, cntrlDeltaGridList); b_layerInfo(p00, 2*L);
    /************************************************************************/
    TimeNodePDE tn05; tn05.i = L-1; tn05.t = L*ht - 0.5*ht;
    TimeNodePDE tn10; tn10.i = L-1; tn10.t = L*ht - 1.0*ht;
    SpaceNodePDE sn05, sn10;
    sn05.i = 0; sn05.x = 0.0; sn10.i = static_cast<int>(N); sn10.x = N*hx;
    for (unsigned int m=0; m<=M; m++)
    {
    ...
    for (unsigned int j=0; j<No; j++)
    {
        ...
        for (unsigned int i=0; i<Nc; i++)
        {
            _w[j] += mOptParameter.k[i][j] * (_p00[i] + 2.0*r*gpi(i, 2*L, u_info, mOptParameter)*sgn(g0i(i,2*L,u_info,mOptParameter)));
        }
    }
    delete [] _p00;
    /************************************************************************/
    ...
    if (use == true) add2Info(p05, p_info, 2*L-1, hx, hy, cntrlDeltaGridList); b_layerInfo(p05, 2*L-1);
    if (use == true) add2Info(p10, p_info, 2*L-2, hx, hy, cntrlDeltaGridList); b_layerInfo(p10, 2*L-2);
    delete [] _w;
    /************************************************************************/
    //------------------------------------- initial conditions -------------------------------------//
    ...
    for (unsigned int ln=L-2; ln != stop; ln--)
    {
        TimeNodePDE tn05; tn05.i = ln+1; tn05.t = tn05.i*ht+0.5*ht;
        TimeNodePDE tn10; tn10.i = ln+1; tn10.t = tn10.i*ht;
        TimeNodePDE tn15; tn15.i = ln;   tn15.t = tn15.i*ht+0.5*ht;
...
auto Problem2HDirichlet2::initConjtWeightMatrix(const std::vector<DoubleMatrix> &u) const -> void
{
    const Dimension dimX = spaceDimension(Dimension::DimensionX);
    const Dimension dimY = spaceDimension(Dimension::DimensionY);
    const Dimension time = timeDimension();
    ...
    Problem2HDirichlet2* const_prob = const_cast<Problem2HDirichlet2*>(this);
    DoubleMatrix &il1 = const_prob->b_init1LayerMatrix;
    DoubleMatrix &il2 = const_prob->b_init2LayerMatrix;

    il1.clear(); il1.resize(M+1, N+1);
    il2.clear(); il2.resize(M+1, N+1);

    for (unsigned int m=0; m<M; m++)
    {
        for (unsigned int n=0; n<N; n++)
        {
            il1[m][n] = -2.0*mu(n,m)*(u[2][m][n]-u[0][m][n])/(2.0*ht);
            il2[m][n] = alpha*il1[m][n] + 2.0*mu(n,m)*u[2][m][n];
        }
    }
}

auto Problem2HDirichlet2::b_initial1(const SpaceNodePDE &sn) const -> double
{
    unsigned int m = static_cast<unsigned int>(sn.j);
    unsigned int n = static_cast<unsigned int>(sn.i);
    return b_init1LayerMatrix[m][n];
}

auto Problem2HDirichlet2::b_initial2(const SpaceNodePDE &sn) const -> double
{
    unsigned int m = static_cast<unsigned int>(sn.j);
    unsigned int n = static_cast<unsigned int>(sn.i);
    return b_init2LayerMatrix[m][n];
}
