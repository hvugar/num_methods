...
void Parabolic1DControl38::main(int argc, char *argv[])
{
    Parabolic1DControl38 hc;
    ...
    //FILE *file = fopen("example334_5.txt", "w");
...
Parabolic1DControl38::Parabolic1DControl38()
{
    alpha = 1.0;
...
double Parabolic1DControl38::fx(const DoubleVector& x)
{
    px = &x;
    DoubleMatrix u;
    IParabolicEquation2D::caluclateMVD(u, h1, h2, ht, N1, N2, M, a1, a2);
...
double Parabolic1DControl38::norm(const DoubleVector& v) const
{
    double nrm = 0.0;
...
void Parabolic1DControl38::gradient(const DoubleVector& x, DoubleVector& g)
{
    px = &x;
    DoubleMatrix u;
    IParabolicEquation2D::caluclateMVD(u, h1, h2, ht, N1, N2, M, a1, a2);
...
void Parabolic1DControl38::calculateGX(const DoubleVector& x, const DoubleMatrix& psi, DoubleVector& g, unsigned int k)
{
    double psiX1;
    double psiX2;
    if (k==1 || k==M)
...
void Parabolic1DControl38::psiDerivative(double &psiX1, double &psiX2, double x1, double x2, const DoubleMatrix &psi)
{
...
double Parabolic1DControl38::initial(unsigned int i, unsigned int j) const
{
    double x1 = i*h1;
    double x2 = j*h2;
    return u(x1, x2, t0);
...
double Parabolic1DControl38::boundary(unsigned int i, unsigned int j, unsigned int k) const
{
    double x1 = i*h1;
    double x2 = j*h2;
    double t  = 0.5*k*ht;
...
double Parabolic1DControl38::f(unsigned int i, unsigned int j, unsigned int k) const
{
    double x1 = i*h1;
    double x2 = j*h2;
    double t  = 0.5*k*ht;
...
double Parabolic1DControl38::binitial(unsigned int i, unsigned int j) const
{
    return -2.0*((*pu)[j][i] - U[j][i]);
}

double Parabolic1DControl38::bboundary(unsigned int i, unsigned int j, unsigned int k) const
{
    return 0.0;
}

double Parabolic1DControl38::bf(unsigned int i, unsigned int j, unsigned int k) const
{
    return 0.0;
}

void Parabolic1DControl38::print(unsigned int i, const DoubleVector& x, const DoubleVector &g, double alpha, RnFunction* fn) const
{
    Parabolic1DControl38 *hc = dynamic_cast<Parabolic1DControl38*>(fn);
    printf("J[%d]: %.16f\n", i, hc->fx(x));
    DoubleVector g1 = g;
    g1.L2Normalize();
    printf("eo: [%12.8f, %12.8f] [%12.8f, %12.8f] [%12.8f, %12.8f]\n", 0.50, 0.80, 0.70, 0.20, 0.20, 0.30);
...
void Parabolic1DControl38::project(DoubleVector &e, int index)
{
    if (index<6)
    {
