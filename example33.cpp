...
void BorderParabolic2D33::main(int argc, char* argv[])
{
    DoubleMatrix m;
    BorderParabolic2D33 bp;
    bp.caluclateMVD(m, bp.h1, bp.h2, bp.ht, bp.N1, bp.N2, bp.M, 1.0, 1.0);
    IPrinter::printMatrix(m);
    //FILE *file = fopen("example33.txt", "w");
    //IPrinter::printMatrix(m, bp.N2, bp.N1, NULL, file);
    //fclose(file);
}

BorderParabolic2D33::BorderParabolic2D33()
{
    x10 = 0.0;
    x11 = 1.0;
...
double BorderParabolic2D33::initial(unsigned int i, unsigned int j) const
{
    double x1 = i*h1;
    double x2 = j*h2;
    return x1*x2;
...
double BorderParabolic2D33::boundary(unsigned int i, unsigned int j, unsigned int k) const
{
    double x1 = i*h1;
    double x2 = j*h2;
    double t  = 0.5*k*ht;
...
double BorderParabolic2D33::f(unsigned int i, unsigned int j, unsigned int k) const
{
    double x1 = i*h1;
    double x2 = j*h2;
    double t  = 0.5*k*ht;
...
double BorderParabolic2D33::v1(double t) const
{
    return 8.0*t;
}

double BorderParabolic2D33::v2(double t) const
{
    return 5.0*t;
}

double BorderParabolic2D33::v3(double t) const
{
    return 6.0*t;
}
