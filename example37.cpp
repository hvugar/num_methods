...
double Parabolic2DControl37::fx(const DoubleVector& v)
{
    pv = &v;
    DoubleMatrix u;
    IParabolicEquation2D::caluclateMVD(u, h1, h2, ht, N1, N2, M, a1, a2);
...
double Parabolic2DControl37::norm(const DoubleVector& v) const
{
    double nrm = 0.0;
    for (unsigned int k=0; k<=M; k++)
    {
...
void Parabolic2DControl37::gradient(const DoubleVector& v, DoubleVector& g)
{
    pv = &v;
    DoubleMatrix u;
    IParabolicEquation2D::caluclateMVD(u, h1, h2, ht, N1, N2, M, a1, a2);
...
void Parabolic2DControl37::calculateGF(const DoubleVector &v, const DoubleMatrix& psi, DoubleVector& g, unsigned int k)
{
...
double Parabolic2DControl37::initial(unsigned int i, unsigned int j) const
{
    double x1 = i*h1;
    double x2 = j*h2;
    return u(x1, x2, t0);
...
double Parabolic2DControl37::boundary(unsigned int i, unsigned int j, unsigned int k) const
{
    double x1 = i*h1;
    double x2 = j*h2;
    double t  = 0.5*k*ht;
...
double Parabolic2DControl37::f(unsigned int i, unsigned int j, unsigned int k) const
{
    double x1 = i*h1;
    double x2 = j*h2;
    //double t  = 0.5*k*ht;
...
double Parabolic2DControl37::binitial(unsigned int i, unsigned int j) const
{
    return -2.0*((*pu)[j][i] - U[j][i]);
}

double Parabolic2DControl37::bboundary(unsigned int i, unsigned int j, unsigned int k) const
{
    return 0.0;
}

double Parabolic2DControl37::bf(unsigned int i, unsigned int j, unsigned int k) const
{
    return 0.0;
}

void Parabolic2DControl37::print(unsigned int i, const DoubleVector &x, const DoubleVector &g, double alpha, RnFunction* fn) const
{
    Parabolic2DControl37 *hc = dynamic_cast<Parabolic2DControl37*>(fn);
    printf("J[%d]: %.16f\n", i, hc->fx(x));
...
void Parabolic2DControl37::project(DoubleVector &e, int index)
{
    if (index<6)
    {
